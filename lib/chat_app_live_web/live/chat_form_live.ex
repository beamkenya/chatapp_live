defmodule ChatAppLiveWeb.ChatFormLive do
  ...
  alias ChatAppLiveWeb.ChatFormComponent
  ...
  def render(assigns) do
    ...
    <div class="flex items-start space-x-4">
      ...
      <div class="min-w-0 flex-1">
        <.live_component module={ChatFormComponent} id={:new} />
      </div>
    </div>
    ...
    <div class="flex items-start space-y-10">
      <ul role="list" class="divide-y divide-gray-100">
        <.message :for={message <- @messages} message={message} message_id={message.id} />
      </ul>
    </div>
    """
  end
    ...
    <li class="flex flex-wrap items-center justify-between gap-x-6 gap-y-4 py-5 sm:flex-nowrap">
      <div>
        <p class="text-sm font-semibold leading-6 text-gray-900">
          <.link navigate={~p"/message/#{@message_id}"} class="hover:underline">
            <%= @message.content %>
          </.link>
        </p>
